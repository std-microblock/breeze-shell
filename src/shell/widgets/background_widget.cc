...
background_widget::background_widget(bool is_main) {
    ...
    if (is_main)
        config::current->context_menu.theme.animation.main_bg.opacity(bg_impl->opacity, 0);
    else {
        config::current->context_menu.theme.animation.submenu_bg.opacity(bg_impl->opacity, 0);
        config::current->context_menu.theme.animation.submenu_bg.x(bg_impl->x, 0);
        config::current->context_menu.theme.animation.submenu_bg.y(bg_impl->y, 0);
        config::current->context_menu.theme.animation.submenu_bg.w(bg_impl->width, 0);
        config::current->context_menu.theme.animation.submenu_bg.h(bg_impl->height, 0);
    }
    bg_impl->opacity->animate_to(255 * config::current->context_menu.theme.background_opacity);
...
void background_widget::render(ui::nanovg_context ctx) {
    auto bg_filler = [this, ctx]() mutable {
        ...
        if (use_self_drawn_border) {
            ...
            NVGcolor shadow_color_from = parse_color(light ? theme.shadow_color_light_from : theme.shadow_color_dark_from),
                     shadow_color_to = parse_color(light ? theme.shadow_color_light_to : theme.shadow_color_dark_to);
            ...
            ctx.fillPaint(ctx.boxGradient(*x + shadow_offset_x,
                                          *y + shadow_offset_y, *width,
                                          *height, corner_radius, shadow_size,
            ...
            auto border_color = light ? theme.border_color_light : theme.border_color_dark;
        ...
        if (theme.inset_border)
            ctx.fillRoundedRect(*x + boarder_width, *y + boarder_width,
                                *width - boarder_width * 2, *height - boarder_width * 2, *radius);
        else
            ctx.fillRoundedRect(*x, *y, *width, *height, *radius);
    };

    ctx.transaction(bg_filler);
    bg_impl->render(ctx);
}
