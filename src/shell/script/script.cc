...
public:
    WindowsThreadWrapper(std::move_only_function<void()> func, size_t stackSize = 0)
            ...
            [&, this, ss = stop_signal]() {
                ...
                for (auto &path : files) {
                    try {
                        ...
                        auto func = JS_Eval(
                            js->ctx, script.c_str(), script.size(),
                        ...
                        if (JS_IsException(func)) {
                            std::cerr << "Syntax Error in file: " << path
                                      << std::endl;
                            auto val =
                                qjs::Value{js->ctx, JS_GetException(js->ctx)};
                        ...
                        auto val =
                            qjs::Value{js->ctx, JS_EvalFunction(js->ctx, func)};
                        if (val.isError()) {
